...
SELECT c.marca,c.modelo, SUM(v.distanciaPercorrida) AS dist--,
		--e.nome
FROM carro c INNER JOIN viagem v ON c.id = v.idCarro
...
SELECT e.nome, SUM(v.distanciaPercorrida) as dist
FROM carro c INNER JOIN viagem v ON c.id = v.idCarro
			 INNER JOIN empresa e ON e.id = c.idEmpresa
			 GROUP BY e.nome
...
SELECT e.nome, SUM(v.distanciaPercorrida) AS dist, CAST(AVG(v.distanciaPercorrida) as decimal(7,2)) AS mediaDist
FROM carro c INNER JOIN viagem v ON c.id = v.idCarro
			 INNER JOIN empresa e ON e.id = c.idEmpresa
			 GROUP BY e.nome
...
SELECT e.nome, MAX(v.distanciaPercorrida)
FROM carro c INNER JOIN viagem v ON c.id = v.idCarro
			 INNER JOIN empresa e ON e.id = c.idEmpresa
GROUP BY e.nome
...
SELECT e.nome, COUNT(c.modelo) AS car
FROM empresa e INNER JOIN carro c ON c.idEmpresa = e.id
GROUP BY e.nome
HAVING COUNT(c.modelo) >= 3
...
SELECT c.marca,c.modelo, e.nome, 
		COUNT(v.distanciaPercorrida) AS viagens 
FROM viagem v INNER JOIN carro c ON v.idCarro = c.id
			  INNER JOIN empresa e ON e.id = c.idEmpresa
GROUP BY c.marca,c.modelo, e.nome
...
SELECT TOP(1) e.nome, c.marca, c.modelo,
	CASE
		WHEN v.distanciaPercorrida < 1000 THEN ' R$ ' + CAST(CAST((v.distanciaPercorrida/1000) * 10 AS decimal (7,2)) AS varchar(20)) 
		WHEN v.distanciaPercorrida > 1000 THEN ' R$ ' + CAST(CAST((v.distanciaPercorrida/1000) * 15 AS decimal (7,2))AS varchar(20)) 
	END AS ValorViagem
FROM viagem v INNER JOIN carro c ON v.idCarro = c.id
			  INNER JOIN empresa e ON e.id = c.idEmpresa
